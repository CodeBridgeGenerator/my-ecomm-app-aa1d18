...
const ProductCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            product: _entity?.product,description: _entity?.description,quantity: _entity?.quantity,category: _entity?.category,
    ...
    return (
        <Dialog header="Create Product" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <InputText id="product" className="w-full mb-3 p-inputtext-sm" value={_entity?.product} onChange={(e) => setValByKey("product", e.target.value)}  required  />
            </span>
